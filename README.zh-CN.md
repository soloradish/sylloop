...
正式安装包包含锁定的 LGPL FFmpeg 可执行文件。Windows 安装包未签名，可能显示 SmartScreen 警告。macOS 应用和内置 FFmpeg 使用 ad-hoc 签名，但未使用 Developer ID 签名或进行公证，因此 macOS 仍可能在首次启动时拦截应用。在 macOS 上，请在 Finder 中按住 Control 点击 Sylloop 并选择**打开**；如果仍被拦截，请打开**系统设置 > 隐私与安全性**并选择**仍要打开**。请从 [GitHub Releases](https://github.com/soloradish/sylloop/releases) 获取安装包，使用发布的 `SHA256SUMS.txt` 进行校验，并在重建内置分析器时使用[精确对应的 FFmpeg 源码包](https://github.com/soloradish/ffmpeg-dist/releases/download/v8.1.2-r1/ffmpeg-8.1.2-r1-sources.tar.xz)。
...
Windows、Apple Silicon 和 Intel macOS 安装包均包含由独立、非官方的 [soloradish/ffmpeg-dist](https://github.com/soloradish/ffmpeg-dist) 发行项目生成且版本锁定的 `core` 构建。精确 release、各架构产物 URL、对应源码包、构建溯源、版本和预期校验值都记录在 `scripts/ffmpeg-lock.json` 中。准备流程会校验归档、`BUILD-INFO.json`、可执行文件架构、版本和 LGPL 兼容配置，并打包完整许可证目录；macOS 准备流程还会为校验后的 FFmpeg 可执行文件应用 ad-hoc 代码签名。开发者可以通过 `FFMPEG_PATH` 让源码构建使用兼容的可执行文件；正式安装包使用内置文件。
...
发布 PR 合并后，在该 `main` 提交上创建并推送 annotated `vX.Y.Z` tag。发布工作流会拒绝版本不一致、轻量 tag、未包含在 `main` 中的 tag，以及已经存在 GitHub Release 的版本。随后它会构建未签名的 Windows NSIS 安装包，以及分别面向 Apple Silicon 和 Intel、使用 ad-hoc 签名但未公证的两个 macOS DMG；工作流会验证应用和 FFmpeg 签名，并在对应原生架构上对每个下载后的安装包执行冒烟测试，随后与 SHA-256 校验和、供 lowid.me 使用的增量兼容 `sylloop-release-v1.json` 清单及 GitHub artifact attestation 一起发布。
